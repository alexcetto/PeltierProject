...
unsigned short readTmp(unsigned char addr)
{
    int temp = 0;
    ...
    if (receive_ack() == 0)  //si on a recu l'ACK, on récupére le contenu des registres 1 & 2 du registre de température
    {
        ...
        I2C_Stop();
        temp = byte1 << 4;  //décale de 4, pour "accueillir" les 4 bits de l'octet2
        temp |= byte2 >> 4; //rajoute les 4 bits de l'octet2
        temp = temp /16; //pour avoir la bonne température
    }
    else
    {
